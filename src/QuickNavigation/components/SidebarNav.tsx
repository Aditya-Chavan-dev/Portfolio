...
export const SidebarNav = ({ onNavigate, activeSection }: SidebarNavProps) => {
    ...
    return (
        <div className="h-full w-16 glass-panel rounded-2xl border-white-10 flex flex-col items-center py-6 gap-section relative overflow-visible z-navigation pointer-events-auto">
            {/* Background Glow */}
            <div className="absolute inset-0 bg-gradient-to-b from-white/5 to-transparent opacity-subtle rounded-2xl pointer-events-none" />

            {/* Icons */}
            <div className="flex flex-col gap-card-content z-10 w-full items-center">
                {navItems.map((item) => {
                    ...
                    return (
                        <button
                            key={item.id}
                            onClick={item.action}
                            className={`w-10 h-10 rounded-xl flex-center transition-fast group relative
                                ${isActive
                                    ? 'bg-gold-dim/20 border border-gold-dim/50 text-gold-glow shadow-glow'
                        ...
                        >
                            ...
                            {/* Tooltip */}
                            <span className="absolute left-full ml-4 px-2 py-1 bg-obsidian border-white-10 rounded-md text-xs font-medium text-white opacity-hover-reveal whitespace-nowrap z-50 pointer-events-none">
                                {item.label}
                            </span>
                        </button>
                    );
